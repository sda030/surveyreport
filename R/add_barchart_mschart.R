...
#'
#' @examples
#' ex_survey1_obj_new <- add_barchart_mschart(ex_survey1_inf)
add_barchart_mschart <- 
	function(obj, 
			...
			function(y_var, y_label, y_group, y_group_label, y_colour_set, y_type, drop_na_y,
				...
				if(type==1L & y_type != "interval") {
					if(!is.na(x_var) && length(y_var)>1) {
						rlang::warn(c("When x_var is specified, barchart1 is currently limited to a single-item chart.",
						...
						ms_percentbar(df = tab_long, y_var = "percent", x_var="var",
		...
		design_frame[[paste0("barchart", type)]] <-
...
#' @export
#' @examples
ms_percentbar <- function(df, y_var, x_var, group_string="category", user_colours=NULL, f_size=8, means=FALSE) {
