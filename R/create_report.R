...
#' create_report(ex_survey1_inf_new,
#'   filepath=file.path(tempdir(), tempfile(fileext = ".docx")))
...
create_report <- 
	function(obj, 
		...
		reportable <- grep("^interpret_|^barchart_|^table_|^caption_", names(obj$design_frame), value = TRUE)
		report_parts <- names(obj$design_frame)[names(obj$design_frame) %in% reportable]
		...
		if(ext=="w") print2.rdocx(x=doc, target = filepath)
