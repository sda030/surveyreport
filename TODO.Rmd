...
-   class-based objects, or just lists for now? input/output as list of df, var_frame and design_frame. Single input, single output.
-   Avoid :: by @importFrom pkg fun to save evaluation time
-   Move "NIFU_Normal første" and other globals to arguments
-   vctr::vec_assert
-   missing() instead of is.na() for arguments?
-   Enumerate options in argument: <https://principles.tidyverse.org/def-enum.html>
-   rlang::`%||%` whenever possible
-   rlang::`%|%` for when replacing NAs
-   Replace informative messages with: rlang::inform(c("this is ", rlang::expr_text(letters)))
...
-   Remove all calls (match.call()), replace with informative error msg
-   rlang::is_installed()
-   Add calls to all objects using rlang?
-   vctrs::%0% for replacing missing
...
-   Better tests and better diagnostics if tests fail: Return tibble
-   Replace separators with patterns? Or character vector patterns? First letters before numbers? First two characters, etc. if(is.integer(var_group_item_sep)) split on first character, etc.
...
-   Rename arguments to snake_case 
